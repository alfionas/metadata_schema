# LA ICPCVD Data Processing Pipeline

This file describes the pipeline to produce Nomad ELN for Large Area Inductively Coupled Plasma Chemical Vapor Deposition (LA ICPCVD) synthesis performed in IMM-CT unit of NFFA-DI. 
...
- `LA ICPCVD_cnr-imm.ct_readme.md`: This file. Describes the Data Processing Pipeline.
- `LA ICPCVD_cnr-imm.ct_ELN.archive.json`: Example Nomad ELN for a LA ICPCVD process.
- `LA ICPCVD_cnr-imm.ct_equipment.archive.json`: Example equipment entry for LA ICPCVD, and entry for LA ICPCVD equipment at Catania IMM.
...
2. **(Optional) load processing monitor file**
   ...
   It might be appropriate to use settings that produce relatively small process monitor files.\
   Alternatively, it might be appropriate to compress such files by any combination of the following strategies: undersample the data; truncate data to e.g. first decimal place; use compressed formats like .zip. See following section for an example Python-based compression script.
